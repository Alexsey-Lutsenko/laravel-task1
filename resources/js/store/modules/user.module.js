...
export default {
    ...
    mutations: {
        ...
        updateUser(state, payload) {
            const i = state.users.indexOf(
                state.users.find((item) => item.id === payload.id)
            );
            if (i >= 0) {
        ...
        remuveUsers(state, payload) {
            const i = state.users.indexOf(
                state.users.find((item) => item.id === payload)
            );
            if (i >= 0) {
    ...
    actions: {
        ...
        async store({ commit, dispatch }, payload) {
            try {
                ...
                const role = store.getters["role/getRoles"].find(
                    (item) => item.id == payload.role_id
                );
        ...
        async remuve({ commit }, payload) {
            try {
                await axios.delete(`api/users/${payload}`);
                commit("remuveUsers", payload);
            } catch (e) {
                commit("addErrors", errorHandler(e));
            }
        },
